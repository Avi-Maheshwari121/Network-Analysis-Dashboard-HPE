...
// Simple StatCard component (no change)
...
export default function ProtocolStatsPage({
  ...
  // Full Metrics History (for PPS, Latency, Jitter charts)
  ...
  // *** CHANGED: Pass all props into the config object ***
  const protocols = [
    { name: 'TCP', data: tcpMetrics, throughputHistory: tcpHistory, latencyHistory: tcpFullMetricsHistory, kpis: tcpKPIs, details: ['latency', 'packet_loss', 'packets_per_second', 'throughput'], fullHistory: tcpFullMetricsHistory },
    { name: 'UDP', data: udpMetrics, throughputHistory: udpHistory, kpis: udpKPIs, details: ['packets_per_second', 'throughput'], fullHistory: udpFullMetricsHistory },
    { name: 'RTP', data: rtpMetrics, throughputHistory: rtpHistory, jitterHistory: rtpFullMetricsHistory, kpis: rtpKPIs, details: ['jitter', 'packet_loss_percentage', 'packets_per_second', 'throughput'], fullHistory: rtpFullMetricsHistory },
    { name: 'QUIC', data: quicMetrics, throughputHistory: quicHistory, kpis: quicKPIs, details: ['packets_per_second', 'throughput'], fullHistory: quicFullMetricsHistory },
    { name: 'DNS', data: dnsMetrics, throughputHistory: dnsHistory, kpis: dnsKPIs, details: ['packets_per_second', 'throughput'], fullHistory: dnsFullMetricsHistory },
    { name: 'IGMP', data: igmpMetrics, throughputHistory: igmpHistory, kpis: igmpKPIs, details: ['packets_per_second', 'throughput'], fullHistory: igmpFullMetricsHistory },
  ];
          ...
          {protocols.map((p) => (
            <label key={p.name} className="flex items-center gap-2 cursor-pointer text-text-main">
              <input
                type="checkbox"
                name={p.name}
                checked={!!visibleProtocols[p.name]}
        ...
        {protocols.map((p) =>
          visibleProtocols[p.name] && (
            <ProtocolMetricsDisplay
              ...
              // *** CHANGED: Pass the correct KPI object ***
              kpis={p.kpis}
              fullMetricsHistory={p.fullHistory}
            />
          )
        )}
      </div>
